...
class SignUpPage extends State<SignUp> {
  ...
  bool isValidFormat(String email, String password, String name, String phoneNumber) {
    if (errorMsg != "") {
      return false;
    }
    return true;
                  ...
                  Container(
                      ...
                      child: Form(
                        ...
                        child: TextFormField(
                          ...
                          inputFormatters: [FilteringTextInputFormatter.deny(KOREAN_WORD_REGEXP)],
                          controller: this.emailController,
                          textAlign: TextAlign.left,
                          decoration: InputDecoration(border: OutlineInputBorder(), hintText: "이메일", hintStyle: TextStyle(fontSize: 14)),
                        ),
                      ))
                ]),
                Row(children: <Widget>[
                  ...
                  Container(
                      ...
                      child: Form(
                        ...
                        child: TextFormField(
                          ...
                          textAlign: TextAlign.left,
                          decoration:
                              InputDecoration(errorMaxLines: 3, border: OutlineInputBorder(), hintText: "비밀번호", hintStyle: TextStyle(fontSize: 14)),
                        ),
                      ))
                ]),
                Row(children: <Widget>[
                  ...
                  Container(
                      ...
                      child: Form(
                        ...
                        child: TextFormField(
                          ...
                          textAlign: TextAlign.left,
                          decoration: InputDecoration(border: OutlineInputBorder(), hintText: "이름", hintStyle: TextStyle(fontSize: 14)),
                        ),
                      ))
                ]),
                Row(children: <Widget>[
                  Container(
                      margin: EdgeInsets.only(right: 8.w, bottom: 8.h, left: 3.w),
                      width: 60.w,
                      height: 20.h,
                      child: FittedBox(fit: BoxFit.fitHeight, child: Text("휴대폰 번호"))),
                  Container(
                      margin: EdgeInsets.only(right: 10.w, bottom: 8.h),
                      width: 242.w,
                      height: 70.h,
                      child: Form(
                        ...
                        child: TextFormField(
                          ...
                          onChanged: (phoneNumber) {
                            if (phoneNumber.length <= 30 && phoneNumberExp.hasMatch(phoneNumber)) {
                              setState(() {
                          ...
                          textAlign: TextAlign.left,
                          decoration:
                              InputDecoration(errorMaxLines: 3, border: OutlineInputBorder(), hintText: "휴대폰 번호", hintStyle: TextStyle(fontSize: 14)),
                        ),
                      ))
                ]),
                Container(
                    ...
                    child: ElevatedButton(
                        style: ButtonStyle(
                          side: MaterialStateProperty.all<BorderSide>(BorderSide(width: 1.w)),
                          backgroundColor: MaterialStateProperty.all(
                            isEmailValid && isPasswordValid && isNameValid && isPhoneNumberValid ? Colors.grey : Colors.white,
                          ),
                          shape: MaterialStateProperty.all<RoundedRectangleBorder>(RoundedRectangleBorder(borderRadius: BorderRadius.circular(15.h))),
                        ),
                        onPressed: () async {
                          var email = emailController.text.toString().replaceAll(' ', '');
                          ...
                          if (!this._emailFormKey.currentState!.validate() ||
                              ...
                              !this._phoneNumberFormKey.currentState!.validate()) {
                            return;
                          }

                          Map data = {"email": email, "password": password, "name": name, "phoneNumber": phoneNumber};
                          var body = json.encode(data);
                          var response = await http.Client().post(Uri.parse("https://${devMode()}.dalbodre.me/api/User/Register"),
                              headers: <String, String>{'Content-Type': 'application/json'}, body: body);

                          Map<String, dynamic> decodedData = json.decode(response.body);

                          if (!decodedData.containsKey("msg") || decodedData["msg"] != SUCCESS_MESSAGE) {
                            ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(duration: const Duration(milliseconds: 1000), content: Text("이미 회원 가입이 된 이메일 혹은 휴대전화입니다. 다시 입력해주세요.")));
                            return;
                          }
                          SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
                          sharedPreferences.setString("signUpToken", decodedData["token"]);

                          ScaffoldMessenger.of(context)
                              .showSnackBar(SnackBar(duration: const Duration(milliseconds: 1000), content: Text("회원가입이 완료되셨습니다.")));
                        ...
                        child: Container(
                            margin: EdgeInsets.only(right: 10.w, bottom: 8.h, top: 10.h, left: 10.w),
                            width: 250.w,
                            height: 30.h,
                            child: FittedBox(
